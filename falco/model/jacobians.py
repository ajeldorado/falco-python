...
def lyot(mp, im, idm):
    ...
    if idm == 1:
        ...
        Edm1pad = pad_crop(Edm1b, mp.dm1.compact.NdmPad)  # Pad or crop for expected sub-array indexing
...
#
                ...
                if mp.coro.upper() in ('LC', 'APLC', 'HLC'):
                    ...
                    EP4sub = falco.prop.mft_f2p(EF3, mp.fl, wvl, mp.F3.compact.dxi, mp.F3.compact.deta, mp.P4.compact.dx, mp.P4.compact.Narr, mp.centering)
                    EP4sub = falco.prop.relay(EP4sub, mp.Nrelay3to4-1, mp.centering)
    ...
    if idm == 2:
        ...
        Edm2inc = pad_crop(falco.prop.ptp(Edm1b, mp.compact.NdmPad*mp.P2.compact.dx, wvl, mp.d_dm1_dm2), mp.dm2.compact.NdmPad)  # E-field incident upon DM2
...
#                dEP2box = ptp_inf_func(dEbox.*Edm2(y_box_AS_ind,x_box_AS_ind), mp.P2.compact.dx*NboxPad2AS,wvl,-1*(mp.d_dm1_dm2 + mp.d_P2_dm1), mp.dm2.dm_spacing, mp.propMethodPTP); # back-propagate to pupil P2
    ...
    if idm == 9:
        ...
        Edm2 = Edm2WFE * DM2stop * np.exp(mirrorFac*2*np.pi*1j*DM2surf/wvl) * \
            falco.prop.ptp(Edm1b, mp.P2.compact.dx*NdmPad, wvl, mp.d_dm1_dm2)
        ...
        if dz2 < 10*wvl:
            ...
            EP2eff = falco.prop.ptp(Edm2, mp.P2.compact.dx*NdmPad, wvl, -dz2)
        ...
        EF3inc = falco.prop.mft_p2f(EP3, mp.fl, wvl, mp.P2.compact.dx, mp.F3.compact.dxi, mp.F3.compact.Nxi, mp.F3.compact.deta, mp.F3.compact.Neta, mp.centering)
        ...
        for iact in mp.dm9.act_ele:
            if np.sum(np.abs(mp.dm9.compact.inf_datacube[:, :, iact])) > 1e-12:  # Only compute for acutators specified for use or for influence functions that are not zeroed out
                ...
                Gzdl[:, Gindex] = mp.dm9.act_sens*(1/stepFac)*mp.dm9.weight*EFend[mp.Fend.corr.maskBool]/np.sqrt(mp.Fend.compact.I00[modvar.sbpIndex])
...
#                    y_box = -1*y_box[::-1]
                ...
                if(Nfft2 > mp.P4.compact.Narr):
                    ...
                    EP4 = pad_crop(mp.P4.compact.croppedMask,Nfft2)*EP4 # Crop the Lyot stop and then apply it.
