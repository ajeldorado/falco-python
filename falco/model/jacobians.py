...
def lyot(mp, im, idm):
    ...
    if(mp.flagApod):
        apodReimaged = falco.util.pad_crop(mp.P3.compact.mask, NdmPad)
    ...
    if any(mp.dm_ind == 1):
        DM1surf = falco.util.pad_crop(mp.dm1.compact.surfM, NdmPad)
    ...
    if any(mp.dm_ind == 2): 
        DM2surf = falco.util.pad_crop(mp.dm2.compact.surfM, NdmPad)
    ...
    if(mp.flagDMwfe): # if(mp.flagDMwfe && (mp.P1.full.Nbeam==mp.P1.compact.Nbeam))
        if any(mp.dm_ind == 1):
            Edm1WFE = np.exp(2*np.pi*1j/wvl*falco.util.pad_crop(mp.dm1.compact.wfe, NdmPad, 'extrapval', 0)) 
        else: 
            Edm1WFE = np.ones((NdmPad, NdmPad))
        if any(mp.dm_ind == 2):
            Edm2WFE = np.exp(2*np.pi*1j/wvl*falco.util.pad_crop(mp.dm2.compact.wfe, NdmPad, 'extrapval', 0)) 
        else: 
    ...
    if(idm==1):
        ...
        if any(mp.dm_ind == 2):
            DM2surf = falco.util.pad_crop(DM2surf, mp.dm1.compact.NdmPad)  
        else:
            DM2surf = np.zeros((mp.dm1.compact.NdmPad, mp.dm1.compact.NdmPad)) 
        if(mp.flagDM2stop):
            DM2stop = falco.util.pad_crop(DM2stop, mp.dm1.compact.NdmPad) 
        else:
            DM2stop = np.ones((mp.dm1.compact.NdmPad, mp.dm1.compact.NdmPad))
        apodReimaged = falco.util.pad_crop( apodReimaged, mp.dm1.compact.NdmPad)
    
        Edm1pad = falco.util.pad_crop(Edm1b, mp.dm1.compact.NdmPad) #--Pad or crop for expected sub-array indexing
        Edm2WFEpad = falco.util.pad_crop(Edm2WFE, mp.dm1.compact.NdmPad) #--Pad or crop for expected sub-array indexing
        ...
        for iact in mp.dm1.act_ele: # np.array([405]): # np.array([1665]): #
            if( np.sum(np.abs(mp.dm1.compact.inf_datacube[:,:,iact]))>1e-12 ):  #--Compute only for influence functions that are not zeroed out
                ...
                dEbox = (mirrorFac*2*np.pi*1j/wvl)*falco.util.pad_crop((mp.dm1.VtoH.reshape(mp.dm1.Nact**2)[iact])*np.squeeze(mp.dm1.compact.inf_datacube[:,:,iact]),NboxPad1AS) #--Pad influence function at DM1 for angular spectrum propagation.
...
#                
                ...
                if mp.coro.upper() in ('LC', 'APLC', 'HLC'):
                    ...
                    EP4noFPM = falco.util.pad_crop(EP4noFPM, mp.P4.compact.Narr) #--Crop down to the size of the Lyot stop opening
    ...
    if(idm==2):
        ...
        apodReimaged = falco.util.pad_crop( apodReimaged, mp.dm2.compact.NdmPad)
        DM2stopPad = falco.util.pad_crop(DM2stop, mp.dm2.compact.NdmPad)
        Edm2WFEpad = falco.util.pad_crop(Edm2WFE, mp.dm2.compact.NdmPad)
    
        #--Propagate full field to DM2 before back-propagating in small boxes
        Edm2inc = falco.util.pad_crop( falco.prop.ptp(Edm1b, mp.compact.NdmPad*mp.P2.compact.dx,wvl, mp.d_dm1_dm2), mp.dm2.compact.NdmPad) # E-field incident upon DM2
        Edm2inc = falco.util.pad_crop(Edm2inc, mp.dm2.compact.NdmPad);
        Edm2 = DM2stopPad*Edm2WFEpad*Edm2inc*np.exp(mirrorFac*2*np.pi*1j/wvl*falco.util.pad_crop(DM2surf, mp.dm2.compact.NdmPad)) # Initial E-field at DM2 including its own phase contribution
        ...
        for iact in mp.dm2.act_ele:
            if( np.sum(np.abs(mp.dm2.compact.inf_datacube[:,:,iact]))>1e-12 ):  #--Only compute for acutators specified for use or for influence functions that are not zeroed out
                ...
                dEbox = (mp.dm2.VtoH.reshape(mp.dm2.Nact**2)[iact])*(mirrorFac*2*np.pi*1j/wvl)*falco.util.pad_crop(np.squeeze(mp.dm2.compact.inf_datacube[:,:,iact]),NboxPad2AS) #--the padded influence function at DM2
...
#        log.warning('GPU support not yet implemented. Proceeding without GPU.')
    ...
    if(idm==1):
        ...
        fftshiftVortex = np.fft.fftshift( falco.mask.falco_gen_vortex_mask( charge, Nfft1) )
...
#                
                ...
                EP3pad = falco.util.pad_crop(EP3, Nfft1)
                ...
                if(Nfft1 > mp.P4.compact.Narr):
                    EP4 = mp.P4.compact.croppedMask*falco.util.pad_crop(EP4, mp.P4.compact.Narr) #--Crop EP4 and then apply Lyot stop 
                else:
                    EP4 = falco.util.pad_crop(mp.P4.compact.croppedMask,Nfft1)*EP4 #--Crop the Lyot stop and then apply it.
                    pass
    ...
    if(idm==2):
        ...
        #--Generate vortex FPM with fftshift already applied
        fftshiftVortex = np.fft.fftshift( falco.mask.falco_gen_vortex_mask( charge, Nfft2) )
...
#                    y_box = -1*y_box[::-1]
                ...
                #--Pad pupil P3 for FFT
                EP3pad = falco.util.pad_crop(EP3, Nfft2)
                ...
                if(Nfft2 > mp.P4.compact.Narr):
                    ...
                    EP4 = falco.util.pad_crop(mp.P4.compact.croppedMask,Nfft2)*EP4 #--Crop the Lyot stop and then apply it.
