...
#def model_full(mp,modvar,**kwargs):
    ...
    if not modvar.zernIndex==1:
        ...
        zernMat = np.squeeze(falco.zern.gen_norm_zern_maps(mp.P1.full.Nbeam,mp.centering,indsZnoll)) #--Cube of normalized (RMS = 1) Zernike modes.
...
#            DM2surf = falco.dms.gen_surf_from_act(mp.dm2, mp.dm2.dx, NdmPad)
    ...
    if(mp.d_P2_dm1 + mp.d_dm1_dm2 == 0):
        ...
        EP2eff = falco.prop.ptp(Edm2,mp.P2.full.dx*NdmPad,wvl,-1*(mp.d_dm1_dm2 + mp.d_P2_dm1)) #--Back propagate to pupil P2
    ...
    if(mp.coro.upper()=='LC' or mp.coro.upper()=='APLC' or mp.coro.upper()=='RODDIER'):
        ...
        EF3inc = falco.prop.mft_p2f(EP3, mp.fl,wvl,mp.P2.full.dx,mp.F3.full.dxi,mp.F3.full.Nxi,mp.F3.full.deta,mp.F3.full.Neta,mp.centering)
        ...
        EP4noFPM = falco.prop.relay(EP3,mp.Nrelay3to4,mp.centering) #--Propagate forward another pupil plane 
        #--MFT from FPM to Lyot Plane (i.e., F3 to P4)
        EP4subtrahend = falco.prop.mft_f2p(EF3,mp.fl,wvl,mp.F3.full.dxi,mp.F3.full.deta,mp.P4.full.dx,mp.P4.full.Narr,mp.centering) # Subtrahend term for Babinet's principle     
        ...
        #--MFT from FPM to Lyot Plane (i.e., F3 to P4)
        EP4 = falco.prop.mft_f2p(EF3,mp.fl,wvl,mp.F3.full.dxi,mp.F3.full.deta,mp.P4.full.dx,mp.P4.full.Narr,mp.centering)  
        EP4 = falco.prop.relay(EP4, mp.Nrelay3to4-1, mp.centering) #--Propagate forward more pupil planes if necessary.

    elif(mp.coro.upper()=='VORTEX' or mp.coro.upper()=='VC' or mp.coro.upper()=='AVC'):
#        if not mp.flagApod:
#            EP3 = falco.utils.pad_crop(EP3,int(2**falco.utils.nextpow2(mp.P1.full.Narr)))
    ...
    EFend = falco.prop.mft_p2f(EP4,mp.fl,wvl,mp.P4.full.dx,mp.Fend.dxi,mp.Fend.Nxi,mp.Fend.deta,mp.Fend.Neta,mp.centering)
...
#         Ein = Ett.*mp.P1.compact.E(:,:,modvar.sbpIndex)
    ...
    if not (modvar.zernIndex==1):
        indsZnoll = modvar.zernIndex #--Just send in 1 Zernike mode
        zernMat = np.squeeze(falco.zern.gen_norm_zern_maps(mp.P1.compact.Nbeam,mp.centering,indsZnoll)) #--Cube of normalized (RMS = 1) Zernike modes.
...
def compact_general(mp, wvl, Ein, normFac, flagEval):
    ...
    if(mp.d_P2_dm1 + mp.d_dm1_dm2 == 0):
        ...
        EP2eff = falco.prop.ptp(Edm2,mp.P2.compact.dx*NdmPad,wvl,-1*(mp.d_dm1_dm2 + mp.d_P2_dm1))
    ...
    if mp.coro.upper()=='LC' or mp.coro.upper()=='APLC' or mp.coro.upper()=='RODDIER':
        ...
        EF3inc = falco.prop.mft_p2f(EP3, mp.fl,wvl,mp.P2.compact.dx,mp.F3.compact.dxi,mp.F3.compact.Nxi,mp.F3.compact.deta,mp.F3.compact.Neta,mp.centering) #--E-field incident upon the FPM
        #--Apply (1-FPM) for Babinet's principle later
        if(mp.coro.upper()=='RODDIER'):
            pass
        ...
        #--MFT from FPM to Lyot Plane (i.e., F3 to P4)
        EP4sub = falco.prop.mft_f2p(EF3,mp.fl,wvl,mp.F3.compact.dxi,mp.F3.compact.deta,mp.P4.compact.dx,mp.P4.compact.Narr,mp.centering) # Subtrahend term for Babinet's principle     
        EP4subRelay = falco.prop.relay(EP4sub,mp.Nrelay3to4-1,mp.centering) #--Propagate forward more pupil planes if necessary.
        #--Babinet's principle at P4
    ...
    #--MFT to camera
    EP4 = falco.prop.relay(EP4,mp.NrelayFend,mp.centering) #--Rotate the final image 180 degrees if necessary
    EFend = falco.prop.mft_p2f(EP4,mp.fl,wvl,mp.P4.compact.dx,dxi,Nxi,deta,Neta,mp.centering);
...
def jacobian(mp):
    ...
    NdmPad = int(mp.compact.NdmPad)
    if(any(mp.dm_ind==1)): 
        mp.dm1.compact.surfM = falco.dms.gen_surf_from_act(mp.dm1, mp.dm1.compact.dx, NdmPad) 
    else: 
        mp.dm1.compact.surfM = np.zeros((NdmPad,NdmPad)) #--Pre-compute the starting DM1 surface
    if(any(mp.dm_ind==2)): 
        mp.dm2.compact.surfM = falco.dms.gen_surf_from_act(mp.dm2, mp.dm2.compact.dx, NdmPad) 
    else:
...
#            results_order = [pool.apply(_func_Jac_ordering, args=(im,idm)) for im,idm in zip(*map(np.ravel, np.meshgrid(np.arange(mp.jac.Nmode,dtype=int),mp.dm_ind))) ]        
    ...
    if(any(mp.dm_ind==1)):
        mp.dm1 = falco.dms.enforce_constraints(mp.dm1) #--Update the sets of tied actuators
