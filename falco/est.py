...
def wrapper(mp, ev, jacStruct):
    ...
    elif mp.estimator.lower() in ['pwp-bp-square', 'pwp-bp', 'pwp-kf']:
...
def perfect(mp, ev):
    ...
    if mp.flagParallel:
        ...
        # #     Ecube[:, iMode, wi] = results[iv]
        # # Eest = np.mean(Ecube, axis=2)  # Average over wavelengths in the subband

        # # Average the E-field over the wavelengths in a subband
        # counter = 0
        # for iMode in range(mp.jac.Nmode):
        #     EsubbandMean = np.zeros((mp.Fend.Neta, mp.Fend.Nxi), dtype=complex)
        #     for wi in range(mp.Nwpsbp):
        #         EsubbandMean += mp.full.lambda_weights[wi] * results[counter]
        #         counter += 1
        ...
        #     isPiston = iZernike == 1
        #     if isPiston:
        #         imageTemp = np.zeros((mp.Fend.Neta, mp.Fend.Nxi), dtype=complex)
        #         imageTemp[mp.Fend.corr.maskBool] = EsubbandMean
        #         ev.imageArray[:, :, 0, iSubband] = imageTemp
