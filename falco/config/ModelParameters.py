...
class ModelParameters:
    ...
    def init_ws(self):
        ...
        mas2lam0D = 1.0/(self.lambda0/self.P1.D*180/np.pi*3600*1000) #Conversion factor: milliarcseconds (mas) to lambda0/D
        ...
        if np.isinf(self.NlamForTT):
            self.Wttlam = np.ones(self.Nsbp,self.Ntt) #Full usage and equal weighting for all T/T's and sub-bandpasses.
        else:
            assert(self.NlamForTT in range(4))
            self.Wttlam = np.zeros((self.Nsbp,self.Ntt)) #Initialize weighting matrix of each tip/tilt-wavelength mode for the controller
            if self.NlamForTT == 3:
                ...
                self.Wttlam[mp.si_ref-1,:] = np.ones((1,self.Ntt))
        ...
        self.Wttlam_ele = np.where(self.Wttlam>0)[0] #Indices of the non-zero control Jacobian modes in the weighting matrix
        ...
        if self.whichPupil == "WFIRST20180103":
            ...
            self.P1.full.mask = falco.masks.falco_gen_pupil_WFIRST_20180103(self.P1.full.Nbeam, self.centering)
            #Generate low-res input pupil for the 'compact' model
            self.P1.compact.mask = falco.masks.falco_gen_pupil_WFIRST_20180103(self.P1.compact.Nbeam, self.centering)


        self.P1.full.Narr = len(self.P1.full.mask) #Total number of pixels across array containing the pupil in the full model. Add 2 pixels to Nbeam when the beam is pixel-centered.
        self.P1.compact.Narr = len(self.P1.compact.mask) #Number of pixels across the array containing the input pupil in the compact model
        ...
        if self.centering == "interpixel":
            self.P2.full.xsDL = np.arange(-(self.P1.full.Narr-1)/2, (self.P1.full.Narr+1)/2)*self.P2.full.dx/self.P2.D
            self.P2.compact.xsDL = np.arange(-(self.P1.compact.Narr-1)/2, (self.P1.compact.Narr+1)/2)*self.P2.compact.dx/self.P2.D
        else:
            self.P2.full.xsDL = np.arange(-self.P1.full.Narr/2, self.P1.full.Narr/2)*self.P2.full.dx/self.P2.D
            self.P2.compact.xsDL = np.arange(-self.P1.compact.Narr/2, self.P1.compact.Narr/2)*self.P2.compact.dx/self.P2.D

        self.P2.full.XsDL, self.P2.full.YsDL = np.meshgrid(self.P2.full.xsDL, self.P2.full.xsDL)
        self.P2.compact.XsDL, self.P2.compact.YsDL = np.meshgrid(self.P2.compact.xsDL, self.P2.compact.xsDL)


        ##DM aperture stops
        self.dm1.full.mask = falco.masks.falco_gen_DM_stop(self.P2.full.dx, self.dm1.Dstop, self.centering)
        self.dm1.compact.mask = falco.masks.falco_gen_DM_stop(self.P2.compact.dx, self.dm1.Dstop, self.centering)

        self.dm2.full.mask = falco.masks.falco_gen_DM_stop(self.P2.full.dx, self.dm2.Dstop, self.centering)
        self.dm2.compact.mask = falco.masks.falco_gen_DM_stop(self.P2.compact.dx, self.dm2.Dstop, self.centering)
        ...
        if self.whichPupil == "WFIRST20180103":
            ...
            self.P4.full.mask = falco.masks.falco_gen_pupil_WFIRSTcycle6_LS(self.P4.full.Nbeam, self.P4.D, self.P4.IDnorm, self.P4.ODnorm, self.LS_strut_width, self.centering, True)
            self.P4.compact.mask = falco.masks.falco_gen_pupil_WFIRSTcycle6_LS(self.P4.compact.Nbeam, self.P4.D, self.P4.IDnorm, self.P4.ODnorm, self.LS_strut_width, self.centering, True)


        assert(not self.coro in ("Vortex","vortex","AVC","VC","LUVOIRA5predef")) #Not inplemented yet
        if not self.coro in ("Vortex","vortex","AVC","VC","LUVOIRA5predef"):
            ...
            while abs(LSdiff) <= 1e-7:
                ...
                LSdiff = LSsum - np.sum(falco.utils.padOrCropEven(self.P4.full.mask, self.P4.full.Narr-2))#Subtract an extra 2 to negate the extra step that overshoots.
                counter = counter + 2

            self.P4.full.croppedMask = falco.utils.padOrCropEven(self.P4.full.mask, self.P4.full.Narr) #The cropped-down Lyot stop for the full model.
            ...
            counter = 2
            while abs(LSdiff) <= 1e-7:
                self.P4.compact.Narr = len(self.P4.compact.mask) - counter #Number of points across the cropped-down Lyot stop
                LSdiff = LSsum - np.sum(falco.utils.padOrCropEven(self.P4.compact.mask, self.P4.compact.Narr - 2)) #Subtract an extra 2 to negate the extra step that overshoots.
                counter = counter + 2
            
            self.P4.compact.croppedMask = falco.utils.padOrCropEven(self.P4.compact.mask, self.P4.compact.Narr) #The cropped-down Lyot stop for the compact model       
            ...
            if self.centering == "interpixel":
                self.P4.compact.xs = np.arange(-(self.P4.compact.Narr-1)/2, (self.P4.compact.Narr+1)/2)*self.P4.compact.dx
            else:
                self.P4.compact.xs = np.arange(-self.P4.compact.Narr/2, self.P4.compact.Narr/2)*self.P4.compact.dx
        ...
        if self.coro in ("LC","DMLC","APLC"):
            ...
            self.F3.full.mask.amp = falco.masks.falco_gen_annular_FPM(self.F3.full.res, self.F3.Rin, self.F3.Rout, self.FPMampFac, self.centering, False)
            ...
            if np.isinf(self.F3.Rout):
                if self.centering == "pixel":
                    self.F3.compact.Nxi = falco.utils.ceil_even((2*(self.F3.Rin*self.F3.compact.res + 0.5)))
                else:
                    self.F3.compact.Nxi = falco.utils.ceil_even((2*self.F3.Rin*self.F3.compact.res))
            else:
                if self.centering == "pixel":
                    self.F3.compact.Nxi = falco.utils.ceil_even((2*(self.F3.Rout*self.F3.compact.res + 0.5)))
                else:
                    self.F3.compact.Nxi = falco.utils.ceil_even((2*self.F3.Rout*self.F3.compact.res))
            ...
            if self.centering == "interpixel" or self.F3.full.Nxi%2 == 1:
                self.F3.full.xisDL  = np.arange(-(self.F3.full.Nxi -1)/2, (self.F3.full.Nxi-1)/2+1)/self.F3.full.res
                self.F3.full.etasDL = np.arange(-(self.F3.full.Neta-1)/2, (self.F3.full.Neta-1)/2+1)/self.F3.full.res
                
                self.F3.compact.xisDL  = np.arange(-(self.F3.compact.Nxi -1)/2, (self.F3.compact.Nxi -1)/2+1)/self.F3.compact.res
                self.F3.compact.etasDL = np.arange(-(self.F3.compact.Neta-1)/2, (self.F3.compact.Neta-1)/2+1)/self.F3.compact.res
            else:
                self.F3.full.xisDL  = np.arange(-self.F3.full.Nxi/2, self.F3.full.Nxi/2)/self.F3.full.res
                self.F3.full.etasDL = np.arange(-self.F3.full.Neta/2, self.F3.full.Neta/2)/self.F3.full.res

                self.F3.compact.xisDL  = np.arange(-self.F3.compact.Nxi/2, self.F3.compact.Nxi/2)/self.F3.compact.res
                self.F3.compact.etasDL = np.arange(-self.F3.compact.Neta/2, self.F3.compact.Neta/2)/self.F3.compact.res
            ...
            self.F3.compact.mask.amp = falco.masks.falco_gen_annular_FPM(self.F3.compact.res, self.F3.Rin, self.F3.Rout, self.FPMampFac, self.centering, False)
        ...
        if not self.coro in ("Vortex","vortex","AVC","VC","LUVOIRA5predef"):
            ...
            if self.centering == "interpixel" or self.F3.compact.Nxi%2 == 1:
                self.F3.compact.xis  = np.arange(-(self.F3.compact.Nxi -1)/2, (self.F3.compact.Nxi -1)/2+1)*self.F3.compact.dxi
                self.F3.compact.etas = np.arange(-(self.F3.compact.Neta-1)/2  (self.F3.compact.Neta-1)/2+1)*self.F3.compact.deta
            else:
                self.F3.compact.xis  = np.arange(-self.F3.compact.Nxi/2, self.F3.compact.Nxi/2)*self.F3.compact.dxi
                self.F3.compact.etas = np.arange(-self.F3.compact.Neta/2, self.F3.compact.Neta/2)*self.F3.compact.deta
        ...
        self.F4.compact.corr = collections.namedtuple("_F4_compact_corr", "mask xisDL etasDL settings inds")
        self.F4.compact.corr.mask, self.F4.compact.xisDL, self.F4.compact.etasDL = falco.masks.falco_gen_SW_mask(**maskCorr)
        self.F4.compact.corr.settings = collections.namedtuple("_F4_compact_corr_settings", maskCorr.keys())(**maskCorr) #Store values for future reference
        maskCorr["pixresFP"] = self.F4.full.res
        self.F4.full.corr = collections.namedtuple("_F4_full_corr", "mask xisDL etasDL settings inds")
        self.F4.full.corr.mask, self.F4.full.xisDL, self.F4.full.etasDL = falco.masks.falco_gen_SW_mask(**maskCorr)
        self.F4.full.corr.settings = collections.namedtuple("_F4_full_corr_settings", maskCorr.keys())(**maskCorr) #Store values for future reference
        ...
        self.F4.compact.score.settings = collections.namedtuple("_F4_compact_score_settings", maskScore.keys())(**maskScore) #Store values for future reference
        ...
        if self.Nsbp > 1:
            ...
            self.sbp_center_vec = np.linspace(self.lambda0*(1-(self.fracBW-self.fracBWsbp)/2), \
                                  self.lambda0*(1+(self.fracBW-self.fracBWsbp)/2), \
                                  self.Nsbp) #vector of central wavelengths for each sub-bandpass
        else:
            self.sbp_center_vec = self.lambda0


        if self.Nwpsbp > 1 and self.Nsbp==1: # Fill the whole band if just one sub-bandpass
            self.lamFac_vec = np.linspace(1-self.fracBWsbp/2.0, \
                                  1+self.fracBWsbp/2.0, \
                                  self.Nwpsbp) # vector of lambda factors about the center wavelength of each sub-bandpass
                              
        elif self.Nwpsbp > 1:
            # Evenly spaced so that in full bandpass no wavelength is weighted more heavily.
            self.lamFac_vec = np.linspace( 1-(self.fracBWsbp/2.0)*(1-1.0/self.Nwpsbp), \
        ...
        XIS,ETAS = np.meshgrid(self.F4.full.xisDL - self.x_planet, self.F4.full.etasDL - self.y_planet)
        ...
        self.maskHMcore = 0*self.FP4.compact.RHOS
        self.maskCore  = 0*self.FP4.compact.RHOS
