...
from utils import * # ceil_even, ceil_odd
...
def falco_gen_dm_poke_cube(dm, mp, dx_dm, flagGenCube=True, **kwds):
    ...
    if not 'flag_hex_array' in dm.keys():
    ...
    if not 'centering' in dm.keys():
        dm['centering'] = 'pixel'

    #%--Compute coordinates of original influence function
    Ninf0 = dm['inf0'].shape # rows, cols ( = (91,91) )
    x_inf0 = (dm['dx_inf0']*np.arange((-Ninf0[1]+1)/2, (Ninf0[1]-1)/2+1)).astype('float64')

    Ndm0 = ceil_even( Ninf0[0] + (dm['Nact'] - 1)*(dm['dm_spacing']/dm['dx_inf0']) )
    dm['NdmMin'] = ceil_even( Ndm0*(dm['dx_inf0']/dx_dm) ) + 2
    ...
    Atmp = np.vstack((Xact.T.flatten(), Yact.T.flatten(), np.zeros((Xact.size,)), np.ones((Xact.size,)) ))
    # Atmp.shape = (4, 2304)
    # save only x,y:
    xy_cent_act = np.matmul(Mrot, Atmp)[:2,:]

    N0 = np.array(Ninf0).max()
    Npad = ceil_odd( np.sqrt(2.)*N0 )
    ...
    Nbox = ceil_even(((Ninf0pad*dm['dx_inf0'])/dx_dm))
    #dm['Nbox'] = Nbox
    ...
    Nmin = ceil_even( np.max(mp['sbp_center_vec']) *
                      np.max(np.abs(
    ...
    NdmPad = ceil_even( ( NboxAS + 2*(1+ (np.max(np.max(np.abs(xy_cent_act)))+0.5)*NpixPerAct)) )
    ...
    if flagGenCube:
        ...
        r_cent_act_box_inM = np.sqrt(xy_cent_act_box_inM[0,:]**2 + xy_cent_act_box_inM[1,:]**2)
