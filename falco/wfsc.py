...
def loop(mp, out):
    ...
    InormHist = cp.zeros((mp.Nitr+1,)); # Measured, mean raw contrast in scoring regino of dark hole.
    ...
    mp.flagCullActHist = cp.zeros((mp.Nitr+1,),dtype=cp.bool)
    ...
    for Itr in range(mp.Nitr):
        ...
        if cp.any(mp.dm_ind==1): 
            ...
            DM1surf = cp.zeros((mp.dm1.compact.Ndm,mp.dm1.compact.Ndm))
    
        if cp.any(mp.dm_ind==2): 
            DM2surf =  falco.dms.falco_gen_dm_surf(mp.dm2, mp.dm2.compact.dx, mp.dm2.compact.Ndm);  
        else: 
            DM2surf = cp.zeros((mp.dm2.compact.Ndm,mp.dm2.compact.Ndm))
        ...
        if mp.flagFiber:
            mp.thput_vec[Itr] = cp.max(thput)
        else:
            mp.thput_vec[Itr] = thput; #--record keeping
        
        #--Compute the current contrast level
        InormHist[Itr] = cp.mean(Im[mp.Fend.corr.maskBool]);
        ...
        if(mp.flagPlot):
            ...
            im1=ax1.imshow(cp.log10(Im),cmap='magma',interpolation='none',extent=[cp.min(mp.Fend.xisDL),cp.max(mp.Fend.xisDL),cp.min(mp.Fend.xisDL),cp.max(mp.Fend.xisDL)])
            ax1.set_title('Stellar PSF: NI=%.2e'%InormHist[Itr])
            ax1.tick_params(labelbottom=False)
            cbar1 = fig.colorbar(im1, ax = ax1)#,shrink=0.95)

            im3 = ax3.imshow(ImSimOffaxis/cp.max(ImSimOffaxis),cmap=plt.cm.get_cmap('Blues'),interpolation='none',extent=[cp.min(mp.Fend.xisDL),cp.max(mp.Fend.xisDL),cp.min(mp.Fend.xisDL),cp.max(mp.Fend.xisDL)])
            ax3.set_title('Off-axis Thput = %.2f%%'%(100*thput))
            cbar3 = fig.colorbar(im3, ax = ax3)
            cbar3.set_ticks(cp.array([0.0, 0.5, 1.0]))
        ...
        if Itr==0:
            ...
            if hasattr(mp,'dm_ind_sched'):
                cvar.flagCullAct = cp.not_equal(mp.dm_ind_sched[Itr], mp.dm_ind_sched[Itr-1]);
            else:
        ...
        if cp.any(mp.dm_ind==1): 
            jacStruct.G1 = jacStruct.G1*cp.moveaxis(cp.tile(mp.WspatialVec[:,None],[mp.jac.Nmode,1,mp.dm1.Nele]),0,-1)
        if cp.any(mp.dm_ind==2): 
            jacStruct.G2 = jacStruct.G2*cp.moveaxis(cp.tile(mp.WspatialVec[:,None],[mp.jac.Nmode,1,mp.dm2.Nele]),0,-1)
        if cp.any(mp.dm_ind==8): 
            jacStruct.G8 = jacStruct.G8*cp.moveaxis(cp.tile(mp.WspatialVec[:,None],[mp.jac.Nmode,1,mp.dm8.Nele]),0,-1)
        if cp.any(mp.dm_ind==9): 
            jacStruct.G9 = jacStruct.G9*cp.moveaxis(cp.tile(mp.WspatialVec[:,None],[mp.jac.Nmode,1,mp.dm9.Nele]),0,-1)
        ...
        if cp.any(mp.dm_ind==1):
            out.dm1.Vpv[Itr] = cp.max(mp.dm1.V) - cp.min(mp.dm1.V)
        ...
        if cp.any(mp.dm_ind==2):
            out.dm2.Vpv[Itr] = cp.max(mp.dm2.V) - cp.min(mp.dm2.V)
        ...
        if(any(mp.dm_ind==1)):
            ...
            rmsSurf_ele = cp.logical_and(RS>=mp.P1.IDnorm/2., RS<=0.5)
            
            DM1surf = falco.dms.falco_gen_dm_surf(mp.dm1, mp.dm1.compact.dx, mp.dm1.compact.Ndm)
            out.dm1.Spv[Itr] = cp.max(DM1surf)-cp.min(DM1surf)
            out.dm1.Srms[Itr] = cp.sqrt(cp.mean(cp.abs( (DM1surf[rmsSurf_ele]) )**2))
        ...
        if(any(mp.dm_ind==2)):
            ...
            DM2surf = falco.dms.falco_gen_dm_surf(mp.dm2, mp.dm2.compact.dx, mp.dm2.compact.Ndm)
            out.dm2.Spv[Itr] = cp.max(DM2surf)-cp.min(DM2surf)
            out.dm2.Srms[Itr] = cp.sqrt(cp.mean(cp.abs( (DM2surf[rmsSurf_ele]) )**2))
        ...
        print('Itr: ', Itr)
        InormHist[Itr+1] = cp.mean(Im[mp.Fend.corr.maskBool]);
...
def falco_est_perfect_Efield_with_Zernikes(mp):
   ...
   Zernike aberrations at the input pupil.
    ...
    if mp.flagMultiproc:
        ...
        Emat = cp.zeros((mp.Fend.corr.Npix, mp.jac.Nmode), dtype=complex)
        ...
        Ecube = cp.zeros((mp.Fend.corr.Npix, mp.jac.Nmode, mp.Nwpsbp), dtype=complex)
        ...
        Emat = cp.mean(Ecube, axis=2) # Average over wavelengths in the subband
        ...
        for im in range(mp.jac.Nmode):
            ...
            EmatSbp = cp.zeros((mp.Fend.corr.Npix, mp.Nwpsbp),dtype=complex)
...
def falco_ctrl(mp,cvar,jacStruct):
    ...
    cvar.GstarG_wsum  = cp.zeros((cvar.NeleAll,cvar.NeleAll)) 
    cvar.RealGstarEab_wsum = cp.zeros((cvar.NeleAll, 1))

    for im in range(mp.jac.Nmode):

        Gmode = cp.zeros((mp.Fend.corr.Npix,1), dtype=complex) #--Initialize a row to concatenate onto
        if(any(mp.dm_ind==1)): Gmode = cp.hstack((Gmode,cp.squeeze(jacStruct.G1[:,:,im])))
        if(any(mp.dm_ind==2)): Gmode = cp.hstack((Gmode,cp.squeeze(jacStruct.G2[:,:,im])))
        if(any(mp.dm_ind==8)): Gmode = cp.hstack((Gmode,cp.squeeze(jacStruct.G8[:,:,im])))
        if(any(mp.dm_ind==9)): Gmode = cp.hstack((Gmode,cp.squeeze(jacStruct.G9[:,:,im])))
        ...
        cvar.GstarG_wsum += mp.jac.weights[im]*cp.real(cp.conj(Gmode).T @ Gmode) 
        ...
        cvar.RealGstarEab_wsum += mp.jac.weights[im]*cp.real(cp.conj(Gmode).T @ Eweighted.reshape(mp.Fend.corr.Npix,1)) #--Apply the Jacobian weights and add to the total.
    
    #--Make the regularization matrix. (Define only the diagonal here to save RAM.)
    cvar.EyeGstarGdiag = cp.max(cp.diag(cvar.GstarG_wsum))*cp.ones(cvar.NeleAll)
    cvar.EyeNorm = cp.max(cp.diag(cvar.GstarG_wsum))
    print('done.')
...
def falco_ctrl_cull(mp, cvar, jacStruct):
    ...
    if(cvar.flagCullAct and cvar.flagRelin):
        ...
        print(cp.nonzero(jacStruct.G1))

        print('Weeding out weak actuators from the control Jacobian...'); 
        if(any(mp.dm_ind==1)):
            G1intNorm = cp.sum( cp.mean(cp.abs(jacStruct.G1)**2,axis=2), axis=0)
            G1intNorm = G1intNorm/cp.max(G1intNorm)
            mp.dm1.act_ele = cp.nonzero(G1intNorm>=10**(mp.logGmin))[0]
            del G1intNorm
        if(any(mp.dm_ind==2)):
            G2intNorm = cp.sum( cp.mean(cp.abs(jacStruct.G2)**2,axis=2), axis=0)
            G2intNorm = G2intNorm/cp.max(G2intNorm)
            mp.dm2.act_ele = cp.nonzero(G2intNorm>=10**(mp.logGmin))[0]
            del G2intNorm
        if(any(mp.dm_ind==8)):
            G8intNorm = cp.sum( cp.mean(cp.abs(jacStruct.G8)**2,axis=2), axis=0)
            G8intNorm = G8intNorm/cp.max(G8intNorm)
            mp.dm8.act_ele = cp.nonzero(G8intNorm>=10**(mp.logGmin))[0]
            del G8intNorm
        if(any(mp.dm_ind==9)):
            G9intNorm = cp.sum( cp.mean(cp.abs(jacStruct.G9)**2,axis=2), axis=0)
            G9intNorm = G9intNorm/cp.max(G9intNorm)
            mp.dm9.act_ele = cp.nonzero(G9intNorm>=10**(mp.logGmin))[0]
            del G9intNorm

        #--Add back in all actuators that are tied (to make the tied actuator logic easier)
        if(any(mp.dm_ind==1)):
            for ti in range(mp.dm1.tied.shape[0]):
                if not (any(mp.dm1.act_ele==mp.dm1.tied[ti,0])):  mp.dm1.act_ele = cp.hstack([mp.dm1.act_ele, mp.dm1.tied[ti,0]])
                if not (any(mp.dm1.act_ele==mp.dm1.tied[ti,1])):  mp.dm1.act_ele = cp.hstack([mp.dm1.act_ele, mp.dm1.tied[ti,1]])
            mp.dm1.act_ele = cp.sort(mp.dm1.act_ele) #--Need to sort for the logic in model_Jacobian.m

        if(any(mp.dm_ind==2)):
            for ti in range(mp.dm2.tied.shape[0]):
                if not any(mp.dm2.act_ele==mp.dm2.tied[ti,0]):  mp.dm2.act_ele = cp.hstack([mp.dm2.act_ele, mp.dm2.tied[ti,0]])
                if not any(mp.dm2.act_ele==mp.dm2.tied[ti,1]):  mp.dm2.act_ele = cp.hstack([mp.dm2.act_ele, mp.dm2.tied[ti,1]])
            mp.dm2.act_ele = cp.sort(mp.dm2.act_ele) #--Need to sort for the logic in model_Jacobian.m
...
def falco_ctrl_grid_search_EFC(mp,cvar):
    ...
    InormVec = cp.zeros(Nvals)

    #--Temporarily store computed DM commands so that the best one does not have to be re-computed
    if(any(mp.dm_ind==1)): dDM1V_store = cp.zeros((mp.dm1.Nact,mp.dm1.Nact,Nvals))
    if(any(mp.dm_ind==2)): dDM2V_store = cp.zeros((mp.dm2.Nact,mp.dm2.Nact,Nvals))
    if(any(mp.dm_ind==8)): dDM8V_store = cp.zeros((mp.dm8.NactTotal,Nvals))
    if(any(mp.dm_ind==9)): dDM9V_store = cp.zeros((mp.dm9.NactTotal,Nvals))
    ...
    indBest = cp.asnumpy(cp.argmin(InormVec))
    cvar.cMin = cp.min(InormVec)
    dDM = falco.config.Object()
    #--delta voltage commands
    if(any(mp.dm_ind==1)): dDM.dDM1V = cp.squeeze(dDM1V_store[:,:,indBest])
    if(any(mp.dm_ind==2)): dDM.dDM2V = cp.squeeze(dDM2V_store[:,:,indBest])
    if(any(mp.dm_ind==8)): dDM.dDM8V = cp.squeeze(dDM8V_store[:,indBest])
    if(any(mp.dm_ind==9)): dDM.dDM9V = cp.squeeze(dDM9V_store[:,indBest])
...
def falco_ctrl_EFC_base(ni,vals_list,mp,cvar):
    ...
    duVecNby1 = -dmfac*cp.linalg.solve( (10**log10reg*cp.diag(cvar.EyeGstarGdiag) + cvar.GstarG_wsum), cvar.RealGstarEab_wsum)
    ...
    if(mp.ctrl.flagUseModel): #--Perform a model-based grid search using the compact model
        ...
        InormAvg = cp.mean(Itotal[mp.Fend.corr.maskBool])
...
def falco_ctrl_setup(mp,cvar):
    ...
    u1 = mp.dm1.V.reshape(mp.dm1.NactTotal)[mp.dm1.act_ele] if(any(mp.dm_ind==1)) else cp.array([])
    u2 = mp.dm2.V.reshape(mp.dm2.NactTotal)[mp.dm2.act_ele] if(any(mp.dm_ind==2)) else cp.array([])
    u8 = mp.dm1.V.reshape(mp.dm8.NactTotal)[mp.dm8.act_ele] if(any(mp.dm_ind==8)) else cp.array([])
    u9 = mp.dm1.V.reshape(mp.dm9.NactTotal)[mp.dm9.act_ele] if(any(mp.dm_ind==9)) else cp.array([])
    cvar.uVec = cp.concatenate((u1,u2,u8,u9))
    cvar.NeleAll = cvar.uVec.size
    
    #--Get the indices of each DM's command within the full command
    u1dummy = 1*cp.ones(mp.dm1.Nele,dtype=int) if(any(mp.dm_ind==1)) else cp.array([])
    u2dummy = 2*cp.ones(mp.dm2.Nele,dtype=int) if(any(mp.dm_ind==2)) else cp.array([])
    u8dummy = 8*cp.ones(mp.dm8.Nele,dtype=int) if(any(mp.dm_ind==8)) else cp.array([])
    u9dummy = 9*cp.ones(mp.dm9.Nele,dtype=int) if(any(mp.dm_ind==9)) else cp.array([])
    cvar.uLegend = cp.concatenate((u1dummy, u2dummy, u8dummy, u9dummy))
...
def falco_ctrl_wrapup(mp,cvar,duVec):
    ...
    if(any(mp.dm_ind==1)):
        dDM1Vvec = cp.zeros(mp.dm1.NactTotal)
    ...
    if(any(mp.dm_ind==2)):  
        dDM2Vvec = cp.zeros(mp.dm2.NactTotal)
    ...
    if(any(mp.dm_ind==8)):  
        dDM.dDM8V = cp.zeros(mp.dm8.NactTotal)
    ...
    if(any(mp.dm_ind==9)):  
        dDM.dDM9V = cp.zeros(mp.dm9.NactTotal)
