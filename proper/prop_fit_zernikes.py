...
def prop_fit_zernikes(wavefront0, pupil0, pupilradius0, nzer, **kwargs):
    ...
    """
    if not "eps" in kwargs: 
        eps = 0.0
    else:
        eps = kwargs["eps"]

    if "eps" in kwargs:
        if eps > 0.0 and nzer > 22:
    ...
    if "fit" in kwargs and kwargs['fit']==True : kval = [0,1]
    else: kval = [0]
    ...
    for k in kval:
        ...
        if k==0:
            ...
            if proper.verbose:
                print("wavefront", wavefront.shape)
        else:
            nw = int(nx) * ny
            r = r.ravel()
            t = t.ravel()
        ...
        if k==0:
            zcoeff = np.linalg.lstsq(ab,wavefront)[0]
        else: ##if k != 0
            fitv = np.zeros((nw)) 
