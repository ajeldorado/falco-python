...
from .prop_hex_aperture import prop_hex_aperture
...
# is the case on most Xeons); when multiprocessing, the best performance is with 4 threads 
# per FFT
...
fftw_multi_nthreads = 4 if fftw_single_nthreads > 4 else fftw_single_nthreads
fftw_use_wisdom = False

if os.path.isfile(os.path.join(lib_dir, '.use_ffti')):
    use_ffti = True
    use_fftw = True
elif os.path.isfile(os.path.join(lib_dir, '.use_fftw')):
    use_fftw = True
