...
def prop_fftw_wisdom(gridsize, direction = 'FFTW_FORWARD'):
    ...
    with open(os.path.join(proper.lib_dir, '.{}pix'.format(str(gridsize)) + '{}threads'.format(str(proper.fftw_single_nthreads)) + '_wisdomfile'), 'wb') as outfile:
    ...
    if proper.fftw_single_nthreads != proper.fftw_multi_nthreads:
        ...
        wisdom = pyfftw.export_wisdom() # Export the wisdom file
        with open(os.path.join(proper.lib_dir, '.{}pix'.format(str(gridsize)) + '{}threads'.format(str(proper.fftw_multi_nthreads)) + '_wisdomfile'), 'wb') as outfile:
            pickle.dump(wisdom, outfile, -1)
