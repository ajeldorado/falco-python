...
mp.eval.indsZnoll = np.array([]) #np.array([2,3,4,5,6]) # Noll indices of Zernikes to compute sensitivities for [1-D ndarray]

####### NEED TO DETERMINE
### Annuli to compute 1nm RMS Zernike sensitivities over. Columns are [inner radius, outer radius]. One row per annulus.
mp.eval.Rsens = np.array([]) # np.array([[3., 4.],[4., 8.]]);  # [2-D ndarray]
...
mp.dm_ind = np.array([1, 2, 9])  # vector of DMs used in controller at ANY time (not necessarily all at once or all the time). 
...
sched_mat = np.tile(np.array([1, 1j, 129, 1, 1]), (4, 1))
...
mp.dt_metal_nm = 1.0  # thickness step size for FPM metal layer (nm)
mp.t_metal_nm_vec = np.arange(0, 120+mp.dt_metal_nm, mp.dt_metal_nm)  # nickel thickness range and sampling (nm)
mp.dt_diel_nm = 1.0  # thickness step size for FPM dielectric layer  (nm)
mp.t_diel_nm_vec = np.arange(0, 900+mp.dt_diel_nm, mp.dt_diel_nm)  #0:mp.dt_diel_nm:900  # PMGI thickness range and sampling (nm)
...
mp.dm9.stepFac = 10  # Adjust the step size in the Jacobian, then divide back out. Used for helping counteract effect of discretization.
...
# mp.dm9.FPMbuffer = 0  # 0.25  # Zero out DM9 actuators too close to the outer edge (within mp.dm9.FPMbuffer lambda0/D of edge)
