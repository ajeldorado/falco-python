...
mp.jac.zerns = np.array([
                            1])  # --Which Zernike modes to include in Jacobian. Given as the max Noll index. Always include the value "1" for the on-axis piston mode.
mp.jac.Zcoef = 1e-9 * np.ones(
    np.size(mp.jac.zerns));  # --meters RMS of Zernike aberrations. (piston value is reset to 1 later)
...
mp.drift.magnitude = 0.008  # --std dev of random walk [V/sqrt(iter)]
...
mp.drift.dm_drift = True
mp.drift.pupil_drift = False
