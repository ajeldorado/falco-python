...
mp.thput_eval_x = 12  # x location [lambda_c/D] in dark hole at which to evaluate throughput
...
mp.source_x_offset_norm = 12  # x location [lambda_c/D] in dark hole at which to compute intensity normalization
...
mp.eval.indsZnoll = np.array([]) #np.array([2, 3]) # Noll indices of Zernikes to compute values for
# Annuli to compute 1nm RMS Zernike sensitivities over. Columns are [inner radius, outer radius]. One row per annulus.
mp.eval.Rsens = np.zeros((0, 2)) #np.array([[3., 4.], [4., 5.], [5., 8.], [8., 9.]]);  # [2-D ndarray]
...
mp.Fend.score.Rout = 20  # outer radius of dark hole scoring region [lambda0/D]
...
mp.SPname = 'SPC-20181220';
SP0 = fits.getdata('../data/WFIRST/PhaseB/SPM_SPC-20181220_1000_rounded9_gray.fits', ext=0)
NbeamSP = 1000
...
# Lyot Stop:
mp.P4.compact.mask = fits.getdata('../data/WFIRST/PhaseB/LS_symm_CGI180718_Str3.20pct_38D91_N120_pixel.fits')
