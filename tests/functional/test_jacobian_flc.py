"""Jacobian accuracy tests for the filtered Lyot coronagraph."""
...
def test_jacobian_flc():
    """Filtered Lyot Jacobian test using FFTs between DMs."""
...
def test_jacobian_flc_no_fpm():
    ...
    indPeak = np.unravel_index(np.argmax(np.abs(Eunpoked2D), axis=None),
                               Eunpoked2D.shape)
    ...
    # print('rmsNormErrorDM1 = %.5f' % rmsNormErrorDM1)
    # print('rmsNormErrorDM2 = %.5f' % rmsNormErrorDM2)
    assert rmsNormErrorDM1 < 1.5e-2
    assert rmsNormErrorDM2 < 1.5e-2


if __name__ == '__main__':
    test_jacobian_flc()
    test_jacobian_flc_no_fpm()
